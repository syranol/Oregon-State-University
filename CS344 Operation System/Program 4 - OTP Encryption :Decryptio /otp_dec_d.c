...
void crackText(char** c_text, char**TBox);
...
void crackText(char** c_text, char**TBox)
{
    int i, textNum, TBoxNum, dNum, l_of_txt;
    
    //* get msg
    l_of_txt = (strlen(*c_text));


    for (i = 0; i < l_of_txt; i++)
    {
        //* get decoded Num
        textNum = conv_To_Num/*double ptr for ref/hash*/(*(*c_text + i));
        TBoxNum = conv_To_Num/*double ptr for ref/hash */(*(*TBox + i));
        
        dNum = ((textNum - TBoxNum) % 27);
...
void c_Process(int con_Port)
{
    ...
    //* Set auth to +
    else if(strcmp(currentCheck, "_") == 0)
    {
